...
class FaceProcessor(threading.Thread):
    ...
    def on_fp_auth_result(self, face_auth_status, detection_faceprint, authenticator):
        ...
        if selected_user is not None:
            ...
            self.send_feedback_msg( f'Success, Matched user: "{selected_user}", Score: {max_score}')
            time.sleep(1)  
            ...
            if self.socket_handler is not None:
                self.socket_handler.broadcast_to_clients(selected_user, self.feedback_gesture)
        else:
    ...
    def face_authenticate_with_gesture(self):
        while True:
            get_feedback_gesture = open(
                './write/feed_gesture_temp.txt', 'r')
            self.feedback_gesture = get_feedback_gesture.readline()
            #print(feedback_gesture)

            if self.feedback_gesture == '1':
                LOGGER.face_rec(f'Gesture Detected: "{self.feedback_gesture}"')
                # LOGGER.face_rec(f'"{attendance}" triggered')
                self.summarized_face_processor_feedback.clear()
                LOGGER.face_rec('Face authentication triggered')
                if len(self.DB_FACEPRINTS) == 0:
                    LOGGER.face_rec('No faceprints detected in sys. Pls Enroll an employee or Resync')
                    self.send_feedback_msg("No faceprints in sys. Pls Enroll an employee or Resync",
                                        FaceDetectionStatus.REJECTED)
                else:
                    with rsid_py.FaceAuthenticator(self.PORT) as authenticator:
                        FaceProcessor.set_device_config(authenticator)
                        self.ready_status_q.put(False)
                        ...
                        authenticator.extract_faceprints_for_auth(
                            on_result=
                            lambda face_auth_status,
                                detection_faceprint: self.on_fp_auth_result(face_auth_status, detection_faceprint,
                                                                            authenticator),
                            on_hint=self.on_hint,
                            on_faces=self.on_faces
                        )
